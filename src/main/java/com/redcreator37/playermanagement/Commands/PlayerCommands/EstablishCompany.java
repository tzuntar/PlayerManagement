...
public class EstablishCompany extends PlayerCommand {
    ...
    public void execute(Player player, String[] args, UUID executor) {
        ...
        Company newCompany = new Company.Builder(Company.NEW_COMPANY_ID, args[0]).build();
        ...
        Bukkit.getScheduler().runTask(PlayerManagement.getPlugin(PlayerManagement.class), () -> {
            try {
                EconomyResponse trans = PlayerManagement.eco.withdrawPlayer(player,
                        PlayerManagement.prefs.establishPrice);
                ...
                newCompany.setBalance(new BigDecimal(PlayerManagement.prefs.establishPrice / 2));
                PlayerManagement.companies.setByName(newCompany);
                player.sendMessage(PlayerManagement.prefs.prefix
                        + MessageFormat.format(Localization.lc("company-registration-successful"), newCompany));
            } catch (SQLException e) {
                player.sendMessage(PlayerManagement.prefs.prefix
                        + MessageFormat.format(Localization.lc("error-saving-company-data"), e.getMessage()));
            }
        });
    }
}
